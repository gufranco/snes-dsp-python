...
def a_finding(command=0x04, args=(0x1234,), first="11" * 8, second="22" * 8, at=0):
    return {
        "command": command,
        "arguments": list(args),
...
class SweepTest(unittest.TestCase):
    ...
    def test_and_what_each_mask_answered(self):
        found = masks.sweep(self._build(differ_on=0x04), commands=(0x04,), arguments=((0x01,),))

        self.assertNotEqual(found[0]["answers"]["dsp1"], found[0]["answers"]["dsp1b"])

    def test_and_where_the_two_answers_first_part_company(self):
        found = masks.sweep(self._build(differ_on=0x04), commands=(0x04,), arguments=((0x01,),))

        self.assertEqual(found[0]["firstDifferingByte"], 0)

    def test_a_command_that_agrees_is_not_reported_beside_one_that_does_not(self):
        found = masks.sweep(
    ...
    def test_the_sweep_reads_as_many_bytes_as_it_was_told_to(self):
        found = masks.sweep(
...
class _Fixed:
    ...
    def __init__(self, part, differ_on):
        self.part = part
...
class CheckTest(unittest.TestCase):
    ...
    def _build(self, first=0x11, second=0x22):
        def build(part):
            return _Answers(first if part == "dsp1" else second)

        return build

    def test_a_divergence_the_parts_still_show_agrees(self):
        pinned = [a_finding()]
    ...
    def test_one_the_parts_no_longer_show_is_a_disagreement(self):
    ...
    def test_and_the_disagreement_names_the_mask_and_both_answers(self):
        pinned = [a_finding(second="99" * 8)]
    ...
    def test_a_divergence_that_has_become_agreement_is_reported_as_that(self):
        pinned = [a_finding()]
    ...
    def test_every_pinned_case_is_counted(self):
...
class FirstDifferenceTest(unittest.TestCase):
    ...
    def test_two_answers_that_differ_at_the_front_part_company_there(self):
        self.assertEqual(masks._first_difference(b"\x01\x02", b"\x99\x02"), 0)

    def test_and_two_that_differ_later_part_company_later(self):
        self.assertEqual(masks._first_difference(b"\x01\x02", b"\x01\x99"), 1)

    def test_two_answers_that_agree_part_company_at_neither_end(self):
        self.assertEqual(masks._first_difference(b"\x01\x02", b"\x01\x02"), 2)

    def test_and_one_shorter_than_the_other_parts_company_where_it_stops(self):
        self.assertEqual(masks._first_difference(b"\x01", b"\x01\x02"), 1)


class StoringTest(unittest.TestCase):
    def test_a_sweep_is_written_where_it_can_be_read_back(self):
        where = Path(tempfile.mkdtemp())

        masks.store([a_finding()], where)

        self.assertEqual(masks.load(where)["divergences"], [a_finding()])

    def test_it_names_which_masks_were_compared(self):
        where = Path(tempfile.mkdtemp())

        masks.store([a_finding()], where)

        self.assertEqual(masks.load(where)["compared"], list(masks.MASKS))

    def test_and_the_digest_of_each_image_that_answered(self):
        where = Path(tempfile.mkdtemp())

        masks.store([a_finding()], where, digest=lambda part: f"digest-of-{part}")

        self.assertEqual(masks.load(where)["images"]["dsp1"], "digest-of-dsp1")

    def test_nothing_written_reads_back_as_nothing(self):
        self.assertIsNone(masks.load(Path(tempfile.mkdtemp())))

    def test_a_sweep_that_found_nothing_still_writes_a_file(self):
        where = Path(tempfile.mkdtemp())

        masks.store([], where)

        self.assertEqual(masks.load(where)["divergences"], [])
...
class ReportTest(unittest.TestCase):
    ...
    def test_a_run_that_agrees_says_how_many_cases_it_re_derived(self):
        self.assertIn("2", " ".join(masks.lines_for(self._found())))

    def test_a_disagreement_names_the_command_and_both_answers(self):
    ...
    def test_a_case_that_has_become_agreement_says_which_command(self):
...
class EntryTest(unittest.TestCase):
    def test_a_machine_missing_one_of_the_masks_says_which(self):
        said = []
    ...
    def test_a_sweep_writes_what_it_found_and_says_how_much_it_tried(self):
        where = Path(tempfile.mkdtemp())
        said = []
        ...
        self.assertEqual(len(masks.load(where)["divergences"]), 1)
        self.assertIn("1 commands x 1 argument sets", " ".join(said))

    def test_nothing_pinned_yet_is_reported_rather_than_passed(self):
        said = []
    ...
    def test_a_pinned_divergence_the_parts_still_show_passes(self):
        where = Path(tempfile.mkdtemp())
        masks.store(
    ...
    def test_and_one_they_no_longer_show_fails(self):
        where = Path(tempfile.mkdtemp())
        masks.store(
            masks.sweep(lambda part: _Fixed(part, 0x04), commands=(0x04,), arguments=((0x01,),)),
            where,
