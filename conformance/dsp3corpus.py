...
Usage:
    ...
    python3 conformance/dsp3corpus.py --record --driver PATH
"""
...
USAGE = "usage: dsp3corpus.py [--corpus PATH] [--record --driver PATH] [--cases N]"
...
def ask(steps, driver):
    ...
    for kind, value in steps:
        ...
        [driver, "dsp3"], input=payload, capture_output=True, check=False, timeout=DRIVER_TIMEOUT
    )
    if done.returncode:
...
def record(driver, wanted):
    ...
    cases = [
        {"seed": seed, "expected": encode(ask(steps_for(seed), driver))}
        for seed in buildable_seeds(wanted)
    ]
    return {
        "comment": (
        ...
        "reference": "snes9x dsp3.cpp, through conformance/ref",
        "cases": cases,
    }
