...
def an_identity(name="Made Up (World).sfc", part="dsp1", title="MADE UP"):
    return {
        "name": name,
        "title": title,
        "part": part,
...
class GroupingTest(unittest.TestCase):
    ...
    def test_a_cartridge_contributes_its_shapes_to_its_own_part(self):
        found = record.gather(
    ...
    def test_two_cartridges_for_one_part_have_their_counts_added(self):
        found = record.gather(
    ...
    def test_and_the_shape_records_how_many_cartridges_used_it(self):
        found = record.gather(
            a_manifest(an_identity(name="one.sfc"), an_identity(name="two.sfc")),
            lambda _name: ("hirom", {"write1 read1": 2}),
        )
    ...
    def test_a_shape_only_one_cartridge_uses_says_so(self):
        def reading(name):
    ...
    def test_every_cartridge_read_is_named_with_what_it_contributed(self):
        found = record.gather(
    ...
    def test_and_carries_the_digests_that_identify_it(self):
        found = record.gather(
            a_manifest(an_identity(name="one.sfc")), lambda _name: ("hirom", {"read1": 1})
        )
    ...
    def test_a_cartridge_that_is_not_here_is_passed_over(self):
        def absent(_name):
            return None

        self.assertEqual(record.gather(a_manifest(an_identity()), absent), {})

    def test_a_cartridge_that_says_nothing_to_its_part_is_a_finding(self):
        found = record.gather(
    ...
    def test_and_is_left_out_of_the_sources_by_default(self):
        found = record.gather(a_manifest(an_identity()), lambda _name: ("hirom", {}))

        self.assertEqual(found, {})

    def test_the_layout_a_cartridge_declares_travels_with_it(self):
...
class PrintingTest(unittest.TestCase):
    def test_what_was_recorded_prints_as_what_it_holds(self):
        found = record.gather(
...
class OrderTest(unittest.TestCase):
    ...
    def test_and_the_busier_of_two_equal_lengths_comes_first(self):
        found = record.gather(
    ...
    def test_every_shape_survives_the_ordering(self):
        found = record.gather(
            a_manifest(an_identity()),
...
class WritingTest(unittest.TestCase):
    ...
    def test_the_file_is_named_after_the_part_it_holds(self):
        self.assertEqual(self._written()["part"], "dsp1")

    def test_it_names_the_tool_that_read_the_cartridges(self):
        self.assertIn("snes-driver-python", self._written()["producedBy"])

    def test_and_every_cartridge_it_was_read_from(self):
        self.assertEqual(len(self._written()["readFrom"]), 1)

    def test_and_the_shapes_themselves(self):
        self.assertEqual(self._written()["shapes"][0]["shape"], "write1 read1")

    def test_no_byte_of_a_cartridge_reaches_the_file(self):
        written = json.dumps(self._written())

        self.assertNotIn("payload", written)
        self.assertNotIn("bytes:", written)

    def test_a_part_that_only_went_silent_writes_no_file(self):
        where = Path(tempfile.mkdtemp())
        found = record.gather(
            a_manifest(an_identity()), lambda _name: ("hirom", {}), keep_silent=True
        )
    ...
    def test_a_part_with_nothing_recorded_writes_no_file(self):
        where = Path(tempfile.mkdtemp())
...
class ReadingTest(unittest.TestCase):
    ...
    def test_a_cartridge_that_is_not_on_this_machine_reads_as_nothing(self):
        self.assertIsNone(record.reading("nowhere.sfc", Path("/nowhere/at/all")))

    def test_a_cartridge_that_is_here_is_confirmed_before_it_is_read(self):
        where = Path(tempfile.mkdtemp())
    ...
    def test_and_what_it_says_comes_back_with_its_layout(self):
        where = Path(tempfile.mkdtemp())
        (where / "made-up.sfc").write_bytes(b"\x00" * 64)
    ...
    def test_a_cartridge_that_is_not_the_one_named_is_refused(self):
        where = Path(tempfile.mkdtemp())
        (where / "made-up.sfc").write_bytes(b"\x00" * 64)
...
class EntryTest(unittest.TestCase):
    def test_a_machine_with_no_cartridge_says_so_rather_than_writing_nothing(self):
        said = []
    ...
    def test_a_run_that_reads_something_reports_what_it_wrote(self):
        where = Path(tempfile.mkdtemp())
        said = []
    ...
    def test_and_names_every_cartridge_that_said_nothing(self):
        said = []
