...
Four cartridge coprocessors, and what they have in common is that none of them is
a chip Nintendo designed. Every one is a NEC uPD77C25 carrying different
microcode, so this repository models what Nintendo added around it and nothing
of what NEC built: the processor itself is
[nec-upd7725-96050-python](https://github.com/gufranco/nec-upd7725-96050-python),
consumed here as a submodule.

The microcode is not carried. It is a Nintendo program, so the repository takes
a copy a reader supplies and refuses one that is not the copy the manifest names.

It is a part rather than a clocked part. It answers the reads and writes a
cartridge performs, and the cycles are spent inside the processor it composes,
which is the member that reports them.

## What this repository is a piece of

Everything below the register interface is the NEC part. The division is
deliberate: a fact about the uPD77C25 has exactly one home, and it is not this
one.
