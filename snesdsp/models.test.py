...
class CatalogueTest(unittest.TestCase):
    ...
    def test_a_part_that_carries_another_part_program_says_so(self):
        self.assertEqual(models.describe("dsp1a").image, "dsp1")

    def test_and_is_still_a_part_in_its_own_right(self):
...
class NamingTest(unittest.TestCase):
    def test_a_part_is_found_by_its_name(self):
        self.assertEqual(models.describe("dsp3").name, "dsp3")

    def test_and_by_any_name_it_is_also_known_by(self):
    ...
    def test_and_the_refusal_names_what_there_is(self):
...
class DeclaredImageTest(unittest.TestCase):
    ...
    def test_and_the_shape_the_processor_needs_to_load_it(self):
        for one in self._manifest()["artifacts"]:
            self.assertEqual(
                one["bytes"], one["programWords"] * 3 + one["dataWords"] * 2, one["part"]
            )

    def test_every_part_of_this_family_runs_the_same_processor(self):
...
class BuildingTest(unittest.TestCase):
    ...
    def test_asking_for_a_part_asks_for_its_microcode(self):
    ...
    def test_the_package_offers_no_way_to_ask_for_anything_but_the_part(self):
        self.assertNotIn("backend", snesdsp.__all__)
        self.assertNotIn("MODELLED", snesdsp.__all__)


def _refused_or_built(name):
